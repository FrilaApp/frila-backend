...
#   2. **Divergência do original** — o espelho bate com o arquivo em BlendOps/Frila.
...
origem="https://api.github.com/repos/BlendOps/Frila/contents/Documentos/API/openapi.yaml?ref=main"
...
if [ -z "$token" ]; then
  echo
  echo "⚠  Original NÃO conferido: BlendOps/Frila é privado e não há token."
  echo "   O espelho pode estar íntegro e mesmo assim atrasado em relação ao contrato."
  echo "   Para ligar: crie um PAT com leitura em BlendOps/Frila e grave como o secret"
...
if ! curl -fsSL -H "Authorization: Bearer $token" \
  ...
  echo
  echo "Token presente, mas não consegui ler o contrato em BlendOps/Frila."
  echo "Confira se o PAT tem leitura de conteúdo nesse repositório."
  exit 1
fi

if diff -q "$tmp" "$espelho" >/dev/null; then
  echo "Espelho em dia com BlendOps/Frila."
  exit 0
fi
