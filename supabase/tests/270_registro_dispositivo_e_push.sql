...
select is(
  has_function_privilege('authenticated',
    'privado.gravar_aceite_push(uuid, timestamptz)', 'execute')
...
-- Simula aceite do FCM (HTTP 200)
do $$
begin
  perform privado.gravar_aceite_push((select id from n_teste), privado.agora() + interval '5 seconds');
end $$;
...
-- Erro transitório abaixo do teto: continua pendente para retry
do $$
begin
...
-- 1. Aceita em 5 segundos após despacho -> dentro dos 60 s
...
values ('c7000000-0000-4000-8000-000000000002', 'cancelamento', gen_random_uuid(),
        privado.agora() - interval '10 minutes', privado.agora() - interval '10 minutes' + interval '5 seconds', 'enviada');

-- 2. Falhou -> fora dos 60s
...
-- 3. Pendente -> fora dos 60s
insert into public.notificacao (usuario_id, tipo, referencia_id, enviada_em, estado_entrega)
values ('c7000000-0000-4000-8000-000000000002', 'cancelamento', gen_random_uuid(),
        privado.agora() - interval '2 minutes', 'pendente');

-- Total na janela de 7 dias: 3. Aceitas em até 60s: 1. Taxa: 33.33%
select is(
  (select taxa_aceite_pct from privado.taxa_aceite_notificacoes_7d()),
  33.33,
  'privado.taxa_aceite_notificacoes_7d calcula 33.33% para 1 aceita em 60s de 3 totais');
