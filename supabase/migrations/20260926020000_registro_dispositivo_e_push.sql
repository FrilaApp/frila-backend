...
begin
  update public.notificacao
     set estado_entrega = 'enviada',
         aceita_em      = coalesce(p_aceita_em, privado.agora()),
         motivo_falha   = null,
         tentativas     = tentativas + 1
   where id = p_notificacao_id;
end $$;

comment on function privado.gravar_aceite_push(uuid, timestamptz) is
  'Grava o aceite da notificação pelo FCM (HTTP 200). estado_entrega passa a enviada e aceita_em é preenchido. Privada, usada pela Edge Function enviar-push.';

revoke execute on function privado.gravar_aceite_push(uuid, timestamptz) from public, anon, authenticated;
grant  execute on function privado.gravar_aceite_push(uuid, timestamptz) to service_role;
...
comment on function privado.gravar_falha_push(uuid, text, boolean, int) is
  'Registra falha de envio ao FCM. Erro permanente ou tentativas >= teto marcam falhou; erro transitório abaixo do teto mantém pendente para retry.';

revoke execute on function privado.gravar_falha_push(uuid, text, boolean, int) from public, anon, authenticated;
grant  execute on function privado.gravar_falha_push(uuid, text, boolean, int) to service_role;
