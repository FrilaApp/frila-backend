...
begin
  -- Check-in manual não confirmado até o fim do turno vira nao_verificado (Critério 1).
  ...
  select count(*) into v_afetados from atualizados;

  -- Transição de posição: confirmada -> cumprida quando o turno terminou e teve check-in registrado
  update public.posicao p
     set estado = 'cumprida'
    from public.turno t
   where t.posicao_id = p.id
...
comment on function privado.fechar_turnos_passados() is
  'Job pós-fim de turno: transforma check-ins manuais não confirmados em nao_verificado e conclui posições cumpridas. Turno sem check-in aguarda decisão 8zLfn0mt.';
