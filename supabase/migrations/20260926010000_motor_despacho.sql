...
begin
  ...
  exception when others then
    -- Só falha de transporte é recuperável: registra sem dado pessoal e deixa a
    -- mensagem na fila para o pg_cron reprocessar.
    select publicado_por into v_autor_id
    from public.vaga
     where id = p_vaga_id;
    ...
    v_autor_id,
    v_autor_id,
    'Falha de transporte no despacho imediato; mensagem mantida na fila para reprocessamento'
    );
    v_req_id := null;
  end;
  return v_req_id;
...
as $$
begin
  -- A configuração ausente ou inválida precisa abortar a operação de forma observável.
  -- Apenas falhas de transporte são capturadas dentro de disparar_despacho.
